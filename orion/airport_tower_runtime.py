...
from orion.airport_runway_coordination import RunwayOperationType, RunwayReservation
...
class AirportTowerController:
    ...
    def assume_runway_control(self, session_id: UUID, *, reason: str) -> None:
        self.core.claim_authority(session_id=session_id, scope=ControllerAuthorityScope.LANDING_AREA, agency=ControllerAgency.AIRPORT_TOWER, reason=reason)
    ...
    def record_ground_boundary_contact(self, session_id: UUID, *, reason: str) -> None:
        ...
        self.core.history.record(session_id=session_id, event_type="ground_tower_boundary_contact", reason=reason, source_agency=ControllerAgency.AIRPORT_TOWER)

    def reserve_runway(self, *, session_id: UUID, runway_id: str, operation: RunwayOperationKind, reason: str) -> RunwayReservation:
        ...
        return self.reservations.reserve(RunwayReservation(session_id=session_id, runway_id=runway_id, operation=operation, reason=reason), runway)
    ...
    def start_departure(self, *, session_id: UUID, runway_id: str) -> TowerDepartureSession:
        ...
        self.core.history.record(session_id=session_id, event_type="tower_departure_started", reason="aircraft holding short for departure", source_agency=ControllerAgency.AIRPORT_TOWER, details={"runway_id": runway_id})
    ...
    def line_up_and_wait(self, session_id: UUID, *, reason: str) -> OperationalInstruction:
        ...
        reservation = self.reserve_runway(session_id=session_id, runway_id=session.runway_id, operation=RunwayOperationKind.LINE_UP, reason=reason)
        session.state = TowerDepartureState.LINE_UP_AND_WAIT
        session.reservation_id = reservation.reservation_id
        self._departures[session_id] = session
        return self._issue_runway_instruction(session_id=session_id, semantic_action="line_up_and_wait", runway_id=session.runway_id)
    ...
    def clear_takeoff(self, session_id: UUID, *, reason: str) -> OperationalInstruction:
        ...
        if session.reservation_id is None:
            reservation = self.reserve_runway(session_id=session_id, runway_id=session.runway_id, operation=RunwayOperationKind.TAKEOFF, reason=reason)
        else:
            reservation = self.reservations.change_operation(runway_id=session.runway_id, session_id=session_id, operation=RunwayOperationKind.TAKEOFF, reason=reason)
        session.reservation_id = reservation.reservation_id
        session.state = TowerDepartureState.TAKEOFF_CLEARED
        self._departures[session_id] = session
        return self._issue_runway_instruction(session_id=session_id, semantic_action="takeoff_clearance", runway_id=session.runway_id)

    def cancel_takeoff_clearance(self, session_id: UUID, *, reason: str) -> TowerDepartureSession:
        session = self._require_departure(session_id)
        ...
        if session.reservation_id is not None:
            self.reservations.release(runway_id=session.runway_id, session_id=session_id, reason=reason)
            session.reservation_id = None
        session.state = TowerDepartureState.TAKEOFF_CLEARANCE_CANCELLED
        self._departures[session_id] = session
    ...
    def begin_takeoff_roll(self, session_id: UUID) -> TowerDepartureSession:
        ...
        self.reservations.advance_commitment(runway_id=session.runway_id, session_id=session_id, commitment=CommitmentState.PHYSICALLY_COMMITTED, reason="takeoff roll observed")
    ...
    def mark_airborne(self, session_id: UUID) -> TowerDepartureSession:
        session = self._require_departure(session_id)
        if session.state is TowerDepartureState.AIRBORNE:
            return session.model_copy(deep=True)
        if session.state is not TowerDepartureState.TAKEOFF_ROLL:
            raise ValueError("Airborne event requires takeoff roll")
        self.reservations.complete(runway_id=session.runway_id, session_id=session_id, reason="airborne event confirmed")
    ...
    def mark_stopped_on_runway(self, session_id: UUID, *, reason: str) -> TowerDepartureSession:
        session = self._require_departure(session_id)
        if session.state is not TowerDepartureState.REJECTED_TAKEOFF:
            raise ValueError("Stopped-on-runway requires rejected takeoff")
        session.state = TowerDepartureState.STOPPED_ON_RUNWAY
        self._departures[session_id] = session
        self._record_departure_state(session, reason)
        return session.model_copy(deep=True)

    def mark_runway_vacated_after_abort(self, session_id: UUID, *, reason: str) -> TowerDepartureSession:
        session = self._require_departure(session_id)
        if session.state is not TowerDepartureState.STOPPED_ON_RUNWAY:
            raise ValueError("Runway-vacated-after-abort requires stopped-on-runway state")
        if session.reservation_id is not None:
            self.reservations.complete(runway_id=session.runway_id, session_id=session_id, reason=reason)
            session.reservation_id = None
        session.state = TowerDepartureState.RUNWAY_VACATED_AFTER_ABORT
        self._departures[session_id] = session
        self._record_departure_state(session, reason)
        return session.model_copy(deep=True)
    ...
    def start_arrival(self, *, session_id: UUID, runway_id: str) -> TowerArrivalSession:
        ...
        self.core.history.record(session_id=session_id, event_type="tower_arrival_started", reason="arrival on final under Tower runway control", source_agency=ControllerAgency.AIRPORT_TOWER, details={"runway_id": runway_id})
        return session.model_copy(deep=True)
    ...
    def clear_landing(self, session_id: UUID, *, reason: str) -> OperationalInstruction:
        ...
        reservation = self.reserve_runway(session_id=session_id, runway_id=session.runway_id, operation=RunwayOperationKind.LANDING, reason=reason)
        session.state = TowerArrivalState.LANDING_CLEARED
        session.reservation_id = reservation.reservation_id
        self._arrivals[session_id] = session
        return self._issue_runway_instruction(session_id=session_id, semantic_action="landing_clearance", runway_id=session.runway_id)
    ...
    def begin_landing_attempt(self, session_id: UUID) -> TowerArrivalSession:
        ...
        self.reservations.advance_commitment(runway_id=session.runway_id, session_id=session_id, commitment=CommitmentState.PHYSICALLY_COMMITTED, reason="landing attempt committed")
    ...
    def mark_runway_vacated(self, session_id: UUID) -> TowerArrivalSession:
        ...
        self.reservations.complete(runway_id=session.runway_id, session_id=session_id, reason="runway-vacated event confirmed")
    ...
    def _issue_runway_instruction(self, *, session_id: UUID, semantic_action: str, runway_id: str) -> OperationalInstruction:
        return self.core.issue_instruction(OperationalInstruction(session_id=session_id, issuing_agency=ControllerAgency.AIRPORT_TOWER, authority_scope=ControllerAuthorityScope.LANDING_AREA, semantic_action=semantic_action, parameters={"runway_id": runway_id}, acknowledgement_required=True, voice_priority=VoicePriority.PROCEDURAL))

    def _record_departure_state(self, session: TowerDepartureSession, reason: str) -> None:
        self.core.history.record(session_id=session.session_id, event_type="tower_departure_state_changed", reason=reason, source_agency=ControllerAgency.AIRPORT_TOWER, details={"state": session.state.value, "runway_id": session.runway_id})

    def _record_arrival_state(self, session: TowerArrivalSession, reason: str) -> None:
        self.core.history.record(session_id=session.session_id, event_type="tower_arrival_state_changed", reason=reason, source_agency=ControllerAgency.AIRPORT_TOWER, details={"state": session.state.value, "runway_id": session.runway_id})
