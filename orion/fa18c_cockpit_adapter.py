...
def normalize_hornet_cockpit_state(payload: object, mapping: HornetArgumentMapping | None = None, profiles: HornetValueProfileSet | None = None) -> HornetCockpitState | None:
    if not isinstance(payload, dict) or payload.get("aircraft_id") not in (None, "fa-18c"):
        return None
    ...
    return HornetCockpitState(
        ...
        requested_comm1_preset=_int_or_none(payload.get("requested_comm1_preset")), requested_comm1_frequency=_float_or_none(payload.get("requested_comm1_frequency")),
        mission_comm2_preset=_int_or_none(payload.get("mission_comm2_preset")), mission_comm2_frequency=_float_or_none(payload.get("mission_comm2_frequency")),
        requested_comm2_preset=_int_or_none(payload.get("requested_comm2_preset")), requested_comm2_frequency=_float_or_none(payload.get("requested_comm2_frequency")),
        left_ddi_page=_string_or_none(payload.get("left_ddi_page")), right_ddi_page=_string_or_none(payload.get("right_ddi_page")),
        mpcd_page=_string_or_none(payload.get("mpcd_page")), sensor_of_interest=_string_or_none(payload.get("sensor_of_interest")), master_mode=_string_or_none(payload.get("master_mode")),
        left_ddi_brightness_raw=_raw(raw_arguments, "left_ddi_brightness"), right_ddi_brightness_raw=_raw(raw_arguments, "right_ddi_brightness"), mpcd_brightness_raw=_raw(raw_arguments, "mpcd_brightness"),
    )


def cockpit_state_for_voice(payload: object, mapping: HornetArgumentMapping | None = None, profiles: HornetValueProfileSet | None = None) -> dict[str, object] | None:
    state = normalize_hornet_cockpit_state(payload, mapping=mapping, profiles=profiles)
    if state is None:
        return None
