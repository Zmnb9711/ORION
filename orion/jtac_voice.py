...
def jtac_session_text(session: JtacSession, language: str = "en") -> str:
    ...
    if session.state is JtacSessionState.ASSIGNED:
        if session.method is JtacDesignationMethod.LASER:
            return f"JTAC назначен. Лазерный код {session.laser_code}." if ru else f"JTAC assigned. Laser code {session.laser_code}."
    ...
    if session.state is JtacSessionState.MARKING:
        if session.method is JtacDesignationMethod.LASER:
            return f"Лазер включён. Код {session.laser_code}." if ru else f"Laser on. Code {session.laser_code}."
    ...
    if session.state is JtacSessionState.CANCELLED:
        return "JTAC задача отменена. Маркировка прекращена." if ru else "JTAC task cancelled. Marking stopped."
...
def submit_jtac_voice(session: JtacSession, language: str = "en") -> VoiceCommand:
    ...
    priority = CommandPriority.HIGH if session.state in {JtacSessionState.MARKING, JtacSessionState.CANCELLED, JtacSessionState.FAILED} else CommandPriority.NORMAL
    return voice_commands.submit(
        VoiceCommandCreate(
            ...
            priority=priority,
            context={"session_id": str(session.session_id), "state": session.state.value, "method": session.method.value, "laser_code": session.laser_code, "target_id": session.target_id, "assigned_asset_id": session.assigned_asset_id},
        )
    )
