...
def _write_state(state: str, *, heard: str = "", reply: str = "", error: str = "") -> None:
    ...
    tmp = path.with_suffix(".tmp"); tmp.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8"); tmp.replace(path)
...
def whisper_stream_path() -> Path:
    ...
    return stt_root() / ("whisper-stream.exe" if os.name == "nt" else "whisper-stream")

def _normalize_transcript_line(line: str) -> str:
    text = ANSI_RE.sub("", line).strip()
    if not text: return ""
    lowered = text.casefold()
    if lowered.startswith(("whisper_", "ggml_", "main:", "system_info:", "init:", "capture:", "processing", "[start speaking]")): return ""
...
def _post_text(text: str, *, core_url: str = CORE_URL, timeout: float = 5.0) -> VoiceBridgeReply:
    ...
    return VoiceBridgeReply(heard=str(payload.get("heard", text)), reply=str(payload.get("reply", "")), matched=bool(payload.get("matched", False)), tts_requested=bool(payload.get("tts_requested", False)))
...
def build_stream_command() -> list[str]:
    ...
    if not stream.is_file(): raise RuntimeError(f"Whisper live microphone component is missing: {stream}. ORION requires whisper-stream in addition to whisper-cli.")
...
def run_forever(*, core_url: str = CORE_URL) -> int:
    try:
        ...
        process = subprocess.Popen(command, cwd=str(whisper_stream_path().parent), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8", errors="replace", bufsize=1, creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0))
        if process.stdout is None: raise RuntimeError("Whisper stream stdout is unavailable")
        _write_state("LISTENING"); last_text = ""
        try:
            for raw_line in process.stdout:
                text = _normalize_transcript_line(raw_line)
                if not text or text.casefold() == last_text.casefold(): continue
                last_text = text; _write_state("HEARD", heard=text)
...
def main() -> int: return run_forever()
