...
def _capture_wav(endpoint: WasapiEndpoint, target: Path, duration_seconds: float = 4.0) -> None:
    import sounddevice as sd
...
def run_conversational_audio_test() -> ConversationalAudioTestResult:
    ...
    with tempfile.TemporaryDirectory(prefix="orion-audio-test-") as tmp:
        ...
        _capture_wav(input_endpoint, capture_path)
        ...
        if not rendered.accepted or not rendered.output_path:
            return ConversationalAudioTestResult(ok=False, recognized_text=recognized, stages=stages, message=rendered.message)
