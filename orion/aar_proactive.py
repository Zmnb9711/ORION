...
class AarProactiveMonitor:
    ...
    def poll(self, language: str = "en") -> AarProactiveUpdate:
        ...
        if session.phase != previous_phase and session.phase == AarPhase.JOIN_UP:
            ...
            self._last_precontact_ready = stability.ready_for_precontact
            ...
            return AarProactiveUpdate(should_announce=True, spoken_text=text, reason="phase_transition", phase=session.phase, guidance=guidance, closure=closure, vertical=vertical, stability=stability)

        if session.phase == AarPhase.JOIN_UP:
            became_ready = stability.ready_for_precontact and not self._last_precontact_ready
            self._last_precontact_ready = stability.ready_for_precontact
            if became_ready:
                self._last_phase = session.phase
                self._last_guidance = guidance
            ...
            if vertical is not None and previous_vertical is not None and vertical.band != previous_vertical:
                ...
                self._last_guidance = guidance
                self._last_closure_band = closure.band if closure is not None else None
                return AarProactiveUpdate(should_announce=True, spoken_text=_vertical_callout(vertical, tanker, language), reason=f"vertical_{vertical.band.value}", phase=session.phase, guidance=guidance, closure=closure, vertical=vertical, stability=stability)
            ...
            if closure_reason is not None and closure is not None and closure.band != previous_band:
                ...
                return AarProactiveUpdate(should_announce=True, spoken_text=text, reason=closure_reason, phase=session.phase, guidance=guidance, closure=closure, vertical=vertical, stability=stability)
        ...
        if reason is None or guidance is None or previous_guidance is None:
            return AarProactiveUpdate(phase=session.phase, guidance=guidance, closure=closure, vertical=vertical, stability=stability)
        ...
        return AarProactiveUpdate(should_announce=True, spoken_text=text, reason=reason, phase=session.phase, guidance=guidance, closure=closure, vertical=vertical, stability=stability)
