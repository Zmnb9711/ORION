...
def create_jtac_session(payload: JtacSessionCreate, speak: bool = Query(default=False), language: str = Query(default="en")) -> JtacSession:
    try:
        session = jtac_sessions.create(payload)
...
def reconcile_jtac_session(session_id: UUID, speak: bool = Query(default=True), language: str = Query(default="en")) -> JtacSession:
    try:
