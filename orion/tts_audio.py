...
def profile_for(command: VoiceCommand, language: str | None = None) -> VoiceProfile:
    ...
    profiles: dict[VoiceAgent, VoiceProfile] = {
        VoiceAgent.ATC: VoiceProfile(profile_id="atc", locale=locale, rate=0.95, radio_effect=True),
        VoiceAgent.AWACS: VoiceProfile(profile_id="awacs", locale=locale, rate=0.92, radio_effect=True),
        VoiceAgent.TANKER: VoiceProfile(profile_id="tanker", locale=locale, rate=0.95, radio_effect=True),
        VoiceAgent.JTAC: VoiceProfile(profile_id="jtac", locale=locale, rate=0.94, radio_effect=True),
        VoiceAgent.MISSION_CONTROL: VoiceProfile(profile_id="mission_control", locale=locale, rate=0.96),
        VoiceAgent.THREAT_ANALYZER: VoiceProfile(profile_id="threat", locale=locale, rate=1.08),
        VoiceAgent.SYSTEM: VoiceProfile(profile_id="orion", locale=locale, rate=1.0),
    }
    return profiles.get(command.agent, VoiceProfile(profile_id=command.agent.value, locale=locale))
