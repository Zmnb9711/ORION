"""Strict SRS 2.4.x TCP/UDP protocol primitives for the reference tester.

The packet layout is derived from DCS-SimpleRadioStandalone 2.4.0.0 at
7694fa9b88889eb4494fd745d31f6f5249c03a0f.  No SRS source is copied here.
"""
...
class SrsProtocolError(ValueError):
    """A malformed or incompatible SRS wire message."""
...
def generate_client_guid() -> str:
    """Return the exact 22-character URL-safe identity used by current SRS."""

    value = base64.urlsafe_b64encode(uuid.uuid4().bytes).decode("ascii").rstrip("=")
    if len(value) != GUID_LENGTH:  # defensive invariant
...
class JsonLineParser:
    """Bounded UTF-8 newline-delimited JSON parser supporting arbitrary recv splits."""
...
def base_client(client_guid: str, name: str, coalition: int = 0) -> dict[str, object]:
    ...
    return {
        ...
        "Name": name.strip() or "ORION YANDEX TEST",
        "Coalition": coalition,
        "AllowRecord": False,
        "Seat": 0,
