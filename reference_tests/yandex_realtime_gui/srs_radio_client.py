"""Headless SRS TCP/UDP client used only by YandexRealtimeTester SRS mode."""
...
class SrsRadioConfig:
    host: str = "127.0.0.1"
    port: int = 5002
    bot_name: str = "ORION YANDEX TEST"
    eam_password: str = ""
...
class SrsRadioClient:
    def __init__(
        ...
        event_callback: RadioEventCallback,
        *,
    ...
    def connect(self) -> None:
        """Perform the bounded TCP/EAM/radio/UDP handshake and start workers."""
        ...
        try:
            ...
            if not compatible_server_version(self.server_version):
                raise SrsProtocolError(
                    f"Unsupported SRS server version {self.server_version!r}; tested compatibility is 2.4.x."
                )
            ...
            if not self.config.eam_password:
                raise ValueError("EAM Password is required when External AWACS Mode is enabled.")

            self._set_state(SrsState.AUTHENTICATING_EAM)
            self._send_tcp(
                build_eam_password_message(
                    self.client_guid, self.config.bot_name, self.config.eam_password
                )
            )
            eam = self._wait_for_message(
                MessageType.EXTERNAL_AWACS_MODE_PASSWORD, HANDSHAKE_TIMEOUT_SECONDS
            )
            ...
            self._emit(
                ...
                client_guid=mask_guid(self.client_guid),
    ...
    def _wait_for_message(self, expected: MessageType, timeout: float) -> dict[str, Any]:
        ...
        while not self.stop_event.is_set():
            if self._pending_tcp:
                message = self._pending_tcp.popleft()
            else:
    ...
    def _start_workers(self) -> None:
        ...
        self.maintenance_thread = threading.Thread(
            target=self._maintenance_worker, name="srs-keepalive", daemon=True
        )
    ...
    def _handle_tcp_message(self, message: dict[str, Any], during_handshake: bool = False) -> None:
        ...
        if not isinstance(raw_type, (int, str)):
            self.tcp_parse_errors += 1
            self._emit("srs.tcp.unexpected", msg_type=raw_type)
            return
        try:
            kind = MessageType(int(raw_type))
    ...
    def _send_keepalive(self) -> None:
        self._send_tcp(
            build_ping_message(self.client_guid, self.config.bot_name, self.coalition)
        )
        udp = self.udp_socket
        if udp is None:
    ...
    def _send_tcp(self, message: dict[str, object]) -> None:
        ...
        if tcp is None:
            raise ConnectionError("SRS TCP socket is unavailable.")
        encoded = encode_tcp_message(message)
        ...
        with self._close_lock:
            ...
            if send_disconnect and self.tcp_socket is not None and self.coalition in {1, 2}:
                try:
                    self._send_tcp(
                        build_eam_disconnect_message(
                            self.client_guid, self.config.bot_name, self.coalition
                        )
                    )
                except OSError:
                    pass
    ...
    def report(self) -> dict[str, object]:
        return {
            ...
            "worker_close_status": dict(self.worker_close_status),
        }
