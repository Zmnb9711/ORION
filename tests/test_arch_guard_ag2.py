...
def _fixture_database(tmp_path: Path) -> Path:
    ...
    lines = [_decision_row(f"D{number:02d}") for number in range(1, 73)]
    document.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    for ordinal, decision_id in enumerate(f"D{number:02d}" for number in range(1, 73)):
        _insert_item(
            connection,
...
def test_all_decisions_and_supersession_rejection_edges_have_provenance(tmp_path: Path) -> None:
    ...
    assert decision_ids == {f"D{number:02d}" for number in range(1, 73)}
