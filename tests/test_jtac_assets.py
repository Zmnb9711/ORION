...
def _unit(unit_id: str, name: str, category: UnitCategory, type_name: str, coalition: Coalition = Coalition.BLUE):
    return MissionUnit(unit_id=unit_id, name=name, category=category, type_name=type_name, coalition=coalition, position=MissionPosition(latitude=0, longitude=0))


def test_selects_friendly_ground_jtac_before_air_asset():
    mission_store.replace(MissionSnapshot(mission_id="m", units=[
        _unit("air", "Kiowa FAC", UnitCategory.HELICOPTER, "OH-58D"),
...
def test_excludes_enemy_and_unrelated_units():
    mission_store.replace(MissionSnapshot(mission_id="m", units=[
