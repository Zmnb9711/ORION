...
def _context() -> LiveMissionContext:
    return LiveMissionContext(
        available=True,
        mission_id="mission-voice",
        ownship=OwnshipContext(aircraft_type="FA-18C_hornet", latitude=41.0, longitude=41.0, altitude_m=5000),
        friendlies=[MissionContact(unit_id="blue-1", name="Ford 2-1", coalition=Coalition.BLUE, latitude=41.05, longitude=41.0, altitude_m=5500, distance_km=5.6, bearing_deg=0.0)],
        hostiles=[MissionContact(unit_id="red-1", name="Bandit 1", coalition=Coalition.RED, latitude=41.0, longitude=41.1, altitude_m=6000, distance_km=8.4, bearing_deg=90.0)],
        awacs=[SupportAsset(unit_id="awacs-1", callsign="Magic", role=DcsRecipientType.AWACS, frequency_mhz=251.0, modulation="AM")],
        tankers=[SupportAsset(unit_id="tanker-1", callsign="Texaco", role=DcsRecipientType.TANKER, frequency_mhz=251.5, modulation="AM")],
        jtac=[SupportAsset(unit_id="jtac-1", callsign="Axeman", role=DcsRecipientType.JTAC, frequency_mhz=133.0, modulation="AM")],
    )
...
def test_nearest_hostile_uses_live_context(monkeypatch) -> None:
    ...
    assert "Bandit 1" in result.spoken_text
    assert "90" in result.spoken_text
...
def test_execution_dispatches_context_query_without_bridge_requirement(monkeypatch) -> None:
    ...
    assert "Magic" in outcome.message
