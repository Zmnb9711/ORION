...
def test_official_99_byte_multiple_frequency_golden_vector() -> None:
    ...
    assert decode_voice_packet(encoded) == value
...
def test_strict_decoder_rejects_structural_corruption(mutator: object, match: str) -> None:
    with pytest.raises(SrsProtocolError, match=match):
        decode_voice_packet(mutator(encode_voice_packet(packet())))  # type: ignore[operator]


def test_strict_decoder_rejects_invalid_frequency_modulation_guid_and_non_finite() -> None:
...
def test_initial_sync_and_radio_update_share_exact_canonical_non_null_radio_info() -> None:
    ...
    radio = build_radio_update_message(GUID, "ORION", 2, 251_000_000.0, radio_state=state)
    ...
    eam = build_eam_password_message(GUID, "ORION", "memory-only")
    assert json.loads(encode_tcp_message(eam))["Version"] == "2.4.0.0"
