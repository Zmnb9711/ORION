...
def test_audio_device_display_map_always_shows_exact_host_api_and_index() -> None:
    mapping = LauncherAudioSectionsMixin._device_display_map(
        [
            {
    ...
    assert "Microphone (Pimax Dream Air) [MME] (#1)" in mapping
    assert "Microphone (Pimax Dream Air) [Windows WASAPI] (#8)" in mapping


def test_selection_text_reports_core_resolution() -> None:
    text = LauncherAudioSectionsMixin._selection_text(
        "mic",
        {
            "device_id": "mic",
            "name": "Microphone",
    ...
    assert text == "PASS — Core active: Microphone [MME] (#1)"
