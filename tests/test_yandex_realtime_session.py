...
def test_raw_segment_hook_and_response_id_suppression_are_independent(
    monkeypatch,
) -> None:  # noqa: ANN001
    ...
    async def send_and_release_suppression(event: dict[str, object]) -> None:
        await original_send(event)
        if event["type"] == "response.cancel":
            suppression["active"] = False
            asyncio.get_running_loop().call_later(0.02, stop.set)

    websocket.send_json = send_and_release_suppression  # type: ignore[method-assign]
    monkeypatch.setattr(aiohttp, "ClientSession", lambda **_: FakeClientSession(websocket))
    stop = threading.Event()
    endpoint = FakeEndpoint(stop, bytes(1764))
    received: list[RealtimeTranscriptSegment] = []
    diagnostics = FakeDiagnostics()

    def accept(segment: RealtimeTranscriptSegment) -> None:
    ...
    asyncio.run(
        YandexRealtimeSession(
            ...
            stop,
            diagnostics,
            on_user_transcript_segment=accept,
            suppress_provider_responses=lambda: suppression["active"],
        ).run()
    )
    assert len(received) == 1
    assert received[0].transcript == "Какая у меня скорость?"
    assert received[0].turn_id == "turn_001"
    assert received[0].event_id == "event-user"
    assert received[0].provider_item_id == "item-user"
    ...
    assert "provider_suppressed_pcm_generated" in diagnostic_names
    assert "provider_suppressed_audio_ignored" in diagnostic_names
    assert "provider_suppressed_text_ignored" in diagnostic_names
    assert "provider_suppressed_response_done" in diagnostic_names


def test_ordinary_server_vad_session_needs_no_physical_semantic_boundary(
    monkeypatch,
) -> None:  # noqa: ANN001
    websocket = FakeWebSocket(b"\x09\x0a" * 120)
    monkeypatch.setattr(aiohttp, "ClientSession", lambda **_: FakeClientSession(websocket))
    stop = threading.Event()
    ...
    asyncio.run(
        YandexRealtimeSession(
            ...
            stop,
            FakeDiagnostics(),
            on_user_transcript_segment=received.append,
        ).run()
    )
    ...
    assert not any(
        event["type"] in {"input_audio_buffer.commit", "response.create"}
        for event in websocket.sent
    )
    assert [segment.transcript for segment in received] == ["Какая у меня скорость?"]
    assert endpoint.events.count(("audio", ("r1", b"\x09\x0a" * 120))) == 1
