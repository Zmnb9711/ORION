...
def test_direct_recognizer_uses_transcript_file_even_with_dirty_console(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    runtime = tmp_path / "whisper.cpp"
    runtime.mkdir()
    cli = runtime / "whisper-cli.exe"
    model = runtime / "models" / "ggml-medium.bin"
    ...
    def fake_run(command, **kwargs):
        ...
        return SimpleNamespace(returncode=0, stdout="� diagnostic", stderr="")
